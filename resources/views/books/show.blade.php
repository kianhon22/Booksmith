    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div class="md:col-span-2 space-y-6">
                            ...
                            <div class="border-t border-gray-200 pt-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-900">{{ $book->category->name }}</p>
                                    </div>
                                    @if($book->publisher)
                                        <div>
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-900">{{ $book->language }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="border-t border-gray-200 pt-4">
                                ...
                                <p class="mt-2 text-sm text-gray-900 leading-relaxed">{{ $book->description }}</p>
                            </div>
                            ...
                            <div class="border-t border-gray-200 pt-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="mt-1 text-4xl font-bold text-indigo-600">${{ number_format($book->price, 2) }}</p>
                                    </div>
                            ...
                            <div class="border-t border-gray-200 pt-4">
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $book->seller->business_name }}</p>
                                <p class="mt-1 text-xs text-gray-500">{{ $book->seller->business_description }}</p>
                            </div>

                            @if(Auth::user()->hasRole('seller') && $book->seller_id !== Auth::id() && $book->stock > 0)
                                <div class="border-t border-gray-200 pt-4">
                            ...
                            @elseif(Auth::user()->hasRole('seller') && $book->seller_id === Auth::id())
                                <div class="border-t border-gray-200 pt-4">
            ...
            @if($relatedBooks->count() > 0)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Related Books</h3>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            @foreach($relatedBooks as $relatedBook)
                                <div class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                                    ...
                                    <div class="p-4">
                                        <a href="{{ route('books.show', $relatedBook) }}">
                                            <h4 class="text-sm font-semibold text-gray-900 hover:text-indigo-600">{{ Str::limit($relatedBook->title, 30) }}</h4>
                                        </a>
                                        <p class="mt-1 text-xs text-gray-600">{{ $relatedBook->author }}</p>
                                        <p class="mt-2 text-lg font-bold text-indigo-600">${{ number_format($relatedBook->price, 2) }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
