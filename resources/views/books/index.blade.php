        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    <div class="mt-4">
                        <button type="submit" form="filter-form" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700">
                            Apply Filters
                        </button>
                    </div>
                </div>
                                ...
                                <div class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                                    ...
                                    <div class="p-4">
                                        <a href="{{ route('books.show', $book) }}" class="block">
                                            <h3 class="text-lg font-semibold text-gray-900 hover:text-indigo-600">{{ Str::limit($book->title, 40) }}</h3>
                                            <p class="mt-1 text-sm text-gray-600">{{ $book->author }}</p>
                                        </a>
                                        <p class="mt-2 text-xs text-gray-500">{{ $book->category->name }}</p>
                                        <div class="mt-3 flex items-center justify-between">
                                            <span class="text-2xl font-bold text-indigo-600">${{ number_format($book->price, 2) }}</span>
                                            <span class="text-xs text-gray-500">{{ $book->stock }} in stock</span>
                                        </div>
                                        <p class="mt-2 text-xs text-gray-600">Sold by: {{ $book->seller->business_name }}</p>
                                        <a href="{{ route('books.show', $book) }}" class="mt-4 block w-full text-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700">
                                            View Details
                                        </a>
                                    </div>
                                </div>
